...
import { AppState, ScannedPage, NumberingStyle, OptionArrangement, HistoryItem } from '../../../lib/tools/text-extract/types';
import { convertPdfToImages, readFileAsBase64, cropImage } from '../../../lib/tools/text-extract/pdfUtils';
import { extractLayoutFromImage } from '../../../lib/tools/text-extract/geminiService';
...
const PdfConverter: React.FC = () => {
  ...
  const [modelId, setModelId] = useState<string>('smart');
  ...
  const startExtraction = async () => {
    ...
    for (let i = 0; i < pagesToProcess.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (page) => {
            ...
            try {
                const elements = await extractLayoutFromImage(page.imageUrl, numberingStyle, includeImages, isBilingual, mcqMode, 0, modelId);
                        ...
                        <div className="flex items-center gap-3">
                         ...
                         <div className="flex items-center gap-2 w-full md:w-auto">
                            ...
                            <select 
                                ...
                                className="px-3 py-2 text-xs bg-indigo-50 border border-indigo-100 text-indigo-700 font-bold rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all"
                            >
                                <option value="smart">Smart Router (Gemini)</option>
                                <option value="openrouter">OpenRouter (Gemma)</option>
                                <option value="modal">Modal (GLM-5)</option>
                            </select>
